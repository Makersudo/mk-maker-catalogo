    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-neutral-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-5xl max-h-full flex flex-col shadow-2xl relative overflow-hidden flex-shrink-0 animate-in fade-in zoom-in-95 duration-200">
          ...
          <form id="productForm" onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex flex-col gap-1.5 md:col-span-2">
                  ...
                  <input required type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]" placeholder="Ex: Batom Matte Rose" />
                </div>

                <div className="flex flex-col gap-1.5 md:col-span-2">
                  <label className="text-xs font-bold text-neutral-700 uppercase tracking-widest">Slug interno</label>
                  <input type="text" value={slug} onChange={(e) => setSlug(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]" placeholder="batom-matte-rose" />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input type="number" min="0" step="1" value={stockQuantity} onChange={(e) => setStockQuantity(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]" placeholder="0" />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-neutral-700 uppercase tracking-widest">Categoria Principal *</label>
                  <select required value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className={`w-full px-4 py-3 ${rootCategories.length === 0 ? 'bg-red-50 border-red-300' : 'bg-neutral-50 border-neutral-200'} border rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]`}>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <select value={subcategoryId} onChange={(e) => setSubcategoryId(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]">
                ...
                <div className="flex flex-col gap-1.5 md:col-span-2">
                  ...
                  <textarea rows={4} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86] resize-none" placeholder="Detalhes do produto, acabamento, tonalidade e uso..." />
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-5">
              ...
              <label className="flex items-center gap-3 p-4 border border-neutral-200 rounded-xl cursor-pointer hover:bg-neutral-50 transition-colors">
                <input type="checkbox" checked={variantsEnabled} onChange={(e) => setVariantsEnabled(e.target.checked)} className="w-5 h-5 text-[#8D514B] rounded focus:ring-[#C98F86] accent-[#C98F86]" />
                <div>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {selectedVariantPreset.values.length > 0 && (
                        <button type="button" onClick={addAllPresetVariants} className="text-xs font-bold text-[#8D514B] hover:text-neutral-950">
                          Adicionar todas
                        </button>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {selectedVariantPreset.values.map((value) => (
                        <button key={value} type="button" onClick={() => addPresetVariant(value)} className="px-3 py-2 rounded-lg border border-neutral-200 bg-white text-xs font-bold text-neutral-700 hover:border-[#E7C9C4] hover:text-[#8D514B]">
                          {value}
                        </button>
                      ))}
                      <button type="button" onClick={() => addPresetVariant()} className="inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-[#E7C9C4] bg-[#F8EEEC] text-xs font-bold text-[#8D514B] hover:bg-[#F3E3DF]">
                    ...
                    {variants.map((variant, index) => (
                      <div key={`${variant.id ?? 'new'}-${index}`} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-3 p-4 rounded-2xl border border-neutral-200 bg-neutral-50">
                        <div className="lg:col-span-2 flex flex-col gap-1.5">
                          ...
                          <input value={variant.options[0]?.name ?? ''} onChange={(e) => updateVariantOption(index, 'name', e.target.value)} className="px-3 py-3 bg-white border border-neutral-200 rounded-lg text-sm focus:outline-none focus:border-[#C98F86]" placeholder={`Ex: ${selectedVariantPreset.optionName}`} />
                        </div>
                        <div className="lg:col-span-2 flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold uppercase tracking-widest text-neutral-500">Valor</label>
                          <input value={variant.options[0]?.value ?? ''} onChange={(e) => updateVariantOption(index, 'value', e.target.value)} className="px-3 py-3 bg-white border border-neutral-200 rounded-lg text-sm focus:outline-none focus:border-[#C98F86]" placeholder={`Ex: ${selectedVariantPreset.placeholderValue}`} />
                        </div>
                        <div className="lg:col-span-3 flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold uppercase tracking-widest text-neutral-500">Nome exibido</label>
                          <input value={variant.label} onChange={(e) => updateVariant(index, { label: e.target.value })} className="px-3 py-3 bg-white border border-neutral-200 rounded-lg text-sm focus:outline-none focus:border-[#C98F86]" placeholder={`Ex: ${selectedVariantPreset.placeholderLabel}`} />
                        </div>
                        <div className="lg:col-span-2 flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold uppercase tracking-widest text-neutral-500">Quantidade</label>
                          <input type="number" min="0" step="1" value={variant.stockQuantity} onChange={(e) => updateVariant(index, { stockQuantity: Number(e.target.value) })} className="px-3 py-3 bg-white border border-neutral-200 rounded-lg text-sm focus:outline-none focus:border-[#C98F86]" placeholder="Ex: 12" />
                        </div>
                        <div className="lg:col-span-2 flex flex-col gap-1.5">
                          <label className="text-[10px] font-bold uppercase tracking-widest text-neutral-500">Preco opcional</label>
                          <input type="number" min="0" step="0.01" value={variant.price ?? ''} onChange={(e) => updateVariant(index, { price: e.target.value === '' ? null : Number(e.target.value) })} className="px-3 py-3 bg-white border border-neutral-200 rounded-lg text-sm focus:outline-none focus:border-[#C98F86]" placeholder="Ex: 99.90" />
                        </div>
                        <div className="lg:col-span-1 flex items-center justify-end gap-2">
                          <label className="flex items-center gap-2 text-xs font-bold text-neutral-600">
                            <input type="checkbox" checked={variant.isActive} onChange={(e) => updateVariant(index, { isActive: e.target.checked })} className="accent-[#C98F86]" />
                            Ativa
                          </label>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input type="text" value={brandLabel} onChange={(e) => setBrandLabel(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]" placeholder="Ex: DIOR, avon, MK MAKER" />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold text-neutral-700 uppercase tracking-widest">Status editorial</label>
                  <select value={catalogStatus} onChange={(e) => setCatalogStatus(e.target.value as 'draft' | 'ready' | 'live')} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input type="text" value={productType} onChange={(e) => setProductType(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]" placeholder="batom, paleta, mascara..." />
                </div>

                <div className="flex flex-col gap-1.5 md:col-span-3">
                  <label className="text-xs font-bold text-neutral-700 uppercase tracking-widest">Variacao</label>
                  <input type="text" value={variation} onChange={(e) => setVariation(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86]" placeholder="Rose, nude, waterproof..." />
                </div>

                <div className="flex flex-col gap-1.5 md:col-span-3">
                  <label className="text-xs font-bold text-neutral-700 uppercase tracking-widest">Caracteristicas</label>
                  <textarea rows={4} value={featuresText} onChange={(e) => setFeaturesText(e.target.value)} className="w-full px-4 py-3 bg-neutral-50 border border-neutral-200 rounded-xl text-sm focus:outline-none focus:border-[#C98F86] focus:ring-1 focus:ring-[#C98F86] resize-none" placeholder="Uma caracteristica por linha" />
                </div>
            ...
            <div className="flex flex-col gap-5">
               ...
               <div className="flex gap-4 overflow-x-auto pb-4 custom-scrollbar">
                 ...
                 <button type="button" onClick={() => fileInputRef.current?.click()} className="shrink-0 w-32 h-32 flex flex-col items-center justify-center gap-2 border-2 border-dashed border-[#E7C9C4] bg-[#F8EEEC] text-[#8D514B] rounded-xl hover:bg-[#F3E3DF] transition-colors">
        ...
        <div className="p-6 border-t border-neutral-100 bg-white grid grid-cols-2 gap-4">
           ...
           <button form="productForm" type="submit" className="px-5 py-3 rounded-xl bg-gradient-to-r from-neutral-950 to-[#A76D65] text-white font-bold text-sm hover:from-neutral-900 hover:to-[#C98F86] transition-colors shadow-md uppercase tracking-widest">
